...
- `.impeccable/config.json` silences four detector rules because each flags a deliberate choice, not a defect: `side-tab` (the accent rule on the settings cards), `repeated-section-kickers` and `all-caps-body` (the mono eyebrow above every section heading), and `design-system-font-size` (DESIGN.md documents the type scale as descriptive ranges, e.g. body copy 14.5–15.5px, not an enumerated token list, so literal size steps in `global.css` will always miss the rule's match). It also carries a narrow `codex-grid-background` ignore scoped to `global.css`, since the `.hero-map` grid texture is the live map's loading placeholder, not decoration. `line-length` is left reporting deliberately: body paragraphs (`.how p`, `.step p`, `.outage p`, `.ref-body`) are capped at `max-width: 70ch`, but the section containers themselves stay full-bleed, so the rule still fires on container width and that's expected.
